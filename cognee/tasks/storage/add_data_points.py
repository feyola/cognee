    ...
    async def _write_edges_grouped(edge_list):
        if per_row_refs:
            ...
            await _add_edges_with_endpoint_repair(
                ...
                nodes,
                edge_list,
                source_ref_key=mapping,
                pipeline_run_id=fold_run_arg,
                repair_nodes=_write_nodes_grouped,
            )
        else:
...
async def _add_edges_with_endpoint_repair(
    ...
    edges: List[tuple],
    *,
    source_ref_key: Optional[str | dict],
    pipeline_run_id: Optional[str],
    repair_nodes: Optional[Callable[[], Awaitable[None]]] = None,
) -> None:
    ...
    try:
        ...
        if repair_nodes is not None:
            # Reapply node ownership, never edge-key mappings or the owner of
            # just one edge group to every node in the batch.
            await repair_nodes()
        else:
            await graph_engine.add_nodes(
                nodes,
...
def _is_foreign_key_violation(error: BaseException) -> bool:
    ...
    while pending:
        ...
        seen.add(id(current))
        if (
            getattr(current, "sqlstate", None) == "23503"
            or getattr(current, "pgcode", None) == "23503"
        ):
            return True
        for related in (
